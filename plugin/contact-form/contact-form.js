...
function submitContactForm() {
  ...
  $.post(
    "plugin/contact-form/contact-form.php",
...
function submitContactFormResponse(response) {
  ...
  $(
    "#contact-form-name,#contact-form-mail,#contact-form-message,#contact-form-send"
  ).qtip("destroy");
    ...
    if (response.info.length) {
      for (var key in response.info) {
        ...
        $("#" + response.info[key].fieldId)
          .qtip({
            style: {
              classes: response.error == 1 ? "qtip-error" : "qtip-success",
            },
